...
function menuInit() {
  document.addEventListener("click", function(e) {
    ...
    } else if (bodyLockStatus && e.target.closest(".menu__link")) {
      bodyUnlock();
...
function initScrollto() {
  document.querySelectorAll('a[href^="#"]').forEach((anchor) => {
    anchor.addEventListener("click", function(e) {
      ...
      if (targetElement) {
        const top = targetElement.getBoundingClientRect().top + window.pageYOffset - 110;
        window.scrollTo({
          top,
          behavior: "smooth"
        });
...
function initSliders() {
  resizableSwiper("(max-width: 767.98px)", ".projects__slider", {
    modules: [],
    observer: true,
    observeParents: true,
    slidesPerView: 1.2,
